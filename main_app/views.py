...
class RoomDetails(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            for room in queryset:
                menu = []
                get_room_obj = Room.objects.get(id=room.id)
                serializer = RoomGetSerializer(get_room_obj)
                data = serializer.data
                for menu_id in serializer.data.get("menu_array"):
                    menu_obj = Menu.objects.get(id=menu_id)
                    menu_serializer = MenuGetSerializer(menu_obj)
                    menu.append(menu_serializer.data)
                data["menu"] = menu
                custom_data.append(data)
            context = custom_response(status.HTTP_200_OK, custom_data, message="Fetched successfully.")
        except Exception as error:
            context = custom_response(status.HTTP_400_BAD_REQUEST, data=str(error))
        return JsonResponse(context, safe=False, status=context.get("status"))
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if serializer.is_valid(raise_exception=False):
                self.perform_create(serializer)
                room_obj = Room.objects.get(id=serializer.data['id'])
                serializer = RoomGetSerializer(room_obj)
                menu = []
                data = serializer.data
                for menu_id in serializer.data.get("menu_array"):
                    menu_obj = Menu.objects.get(id=menu_id)
                    ...
                    menu.append(menu_serializer.data)
                data["menu"] = menu
                context = custom_response(status.HTTP_201_CREATED, data, message="Created Successfully.")
            else:
                context = custom_response(status.HTTP_202_ACCEPTED, serializer.errors, message="Unsuccessful.")
        except Exception as error:
            context = custom_response(status.HTTP_400_BAD_REQUEST, data=str(error))
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            try:
                menu = []
                get_room_obj = Room.objects.get(id=pk)
                serializer = RoomGetSerializer(get_room_obj)
                data = serializer.data
                for menu_id in serializer.data.get("menu_array"):
                    menu_obj = Menu.objects.get(id=menu_id)
                    ...
                    menu.append(menu_serializer.data)
                data["menu"] = menu
                context = custom_response(status.HTTP_200_OK, data, "Fetched Successfully.")
            except Exception as error:
                context = custom_response(status.HTTP_404_NOT_FOUND)
        except Exception as error:
            context = custom_response(status.HTTP_400_BAD_REQUEST, data=str(error))
...
class Guest(ModelViewSet):
    ...
    serializer_class = GuestSerializerGet
        ...
        try:
            ...
            if user_id:
                ...
                if users:
                    ...
                    queryset = Guests.objects.filter(hotel_id__in=hotel_ids)
                    serializer = GuestSerializerGet(queryset, many=True)
    ...
    def create(self, request, *args, **kwargs):
        data, context = [], {}
        try:
            serializer = GuestSerializerEdit(data=request.data)
            if serializer.is_valid():
                self.perform_create(serializer)
    ...
    def partial_update(self, request, pk):
        ...
        try:
            try:
                queryset = Guests.objects.all()
                guest = get_object_or_404(queryset, pk=pk)
                serializer = GuestSerializerEdit(guest, data=request.data, partial=True)
                if serializer.is_valid():
                    serializer.save()
                    guest_obj = Guests.objects.get(id=serializer.data['id'])
                    serializer = GuestSerializerGet(guest_obj)
                    context = custom_response(status.HTTP_200_OK, serializer.data, message="Updated Successfully.")
                else:
                    context = custom_response(status.HTTP_202_ACCEPTED, serializer.errors, message="Unsuccessful.")
            except Exception as error:
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            try:
                ...
                serializer = GuestSerializerGet(get_guest)
                context = custom_response(status.HTTP_200_OK, serializer.data, "Fetched Successfully.")
            except Exception as error:
                context = custom_response(status.HTTP_404_NOT_FOUND)
        except Exception as error:
        ...
        try:
            ...
            if user_id is not None:
                try:
                    ...
                    if payment_status:
                        if payment_status.lower() == 'false':
                            queryset = Hotel.objects.filter(user=user_id, payment_status=payment_status.capitalize()).first()
                            serializer = HotelSerializer(queryset)
                            context = custom_response(status.HTTP_200_OK, serializer.data,
                                                  message="Fetched successfully.")
                    else:
                        queryset = Hotel.objects.filter(user=user_id)
                        serializer = HotelSerializer(queryset, many=True)
                        context = custom_response(status.HTTP_200_OK, serializer.data,
                                                      message="Fetched successfully.")
                except MultipleObjectsReturned as error:
    ...
    def post(self, request):
        try:
            ...
            for obj in data:
                ...
                # menu_category_ids.append(menu_category.id)
...
class DeleteFlatData(APIView):

    def post(self, request):
        try:
            hotel_id = request.data.get("hotel_id")
            menu_ids = set()
            manu_category_ids = set()
            items_id = set()
            item_obj = Items.objects.filter(hotel=hotel_id, is_uploaded=True)
            for item in item_obj:
                items_id.add(item.id)
                menu_ids.add(item.menu.id)
                manu_category_ids.add(item.menu_category.id)
            Items.objects.filter(id__in=list(items_id)).delete() and \
                MenuCategory.objects.filter(id__in=list(manu_category_ids)).delete() and \
                Menu.objects.filter(id__in=list(menu_ids)).delete()
            context = custom_response(status.HTTP_200_OK, message="Data deleted successfully.")
        except Exception as error:
            context = custom_response(status.HTTP_400_BAD_REQUEST, data=str(error))
        return JsonResponse(context, status=context.get('status'), safe=False)
...
# class GetGeneralManagerRole(APIView):
#
#     def get(self, request):
#         try:
...
#             context = custom_response(status.HTTP_200_OK, data, message="General manager role fetched successfully.")
#         except Exception as error:
#             context = custom_response(status.HTTP_400_BAD_REQUEST, data=str(error))
#         return JsonResponse(context, safe=False, status=context.get("status"))
